...
//                 `${import.meta.env.VITE_BACKEND_API}/v1/teacher/resetPassword`,
//                 item,
//                 {
...
const ResetPassword = () => {
    ...
    useEffect(() => {
        ...
        if (storedRememberMe && storedEmail && storedPassword && storedDob) {
    ...
    const sendOtp = async () => {
        try {
            ...
            const formattedDate = dob.split("-").reverse().join("-");
            const item = { email, dob: formattedDate };
            ...
            setSnackbarMessage("Invalid Credentials! Please try again later.");
            setSnackbarOpen(true);
            console.error("An error occurred during OTP sending", error);
        } finally {
            setLoading(false);
        }
    };
